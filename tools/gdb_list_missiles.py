"""List live Samus missile instances and their motion without mutation."""
...
for index in range(array_length(runner["instances"])):
    ...
    if name != "oMissile":
        continue
    print(
        "MISSILE_INSTANCE index=%d id=%d x=%.3f y=%.3f previous=%.3f,%.3f speed=%.3f,%.3f active=%d destroyed=%d"
        % (
            index,
            int(instance["instanceId"]),
            float(instance["x"]),
            float(instance["y"]),
            float(instance["xprevious"]),
            float(instance["yprevious"]),
            float(instance["hspeed"]),
            float(instance["vspeed"]),
            1 if bool(instance["active"]) else 0,
            1 if bool(instance["destroyed"]) else 0,
        )
    )
    found += 1
print("MISSILE_COUNT value=%d" % found)
